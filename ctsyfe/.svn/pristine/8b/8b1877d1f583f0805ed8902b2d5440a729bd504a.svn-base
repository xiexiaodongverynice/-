angular.module('starter.ZhongYiYangShengNewControllers', ['ionic'])
    ...
    .controller('ssysC', function (wxApi, $ionicViewSwitcher, $scope, $timeout, $state, $stateParams, XywyService, Popup, Outlet, $ionicScrollDelegate, $window, $rootScope, geoLocation, $ionicModal, $ionicLoading, GoZzJbYp) {
        ...
        $scope.doQuery = function () {
            ...
            $scope.isShowPx = $scope.isShowPx.map(close);//隐藏所有菜单
            event.stopPropagation()
        };
    ...
    .controller('ssDetailC', function ($scope, $stateParams, XywyService, Popup, $window, GoZzJbYp, $state) {
        $scope.isIos = false;
        if (!!$window.navigator.userAgent.match(/\(i[^;]+;( U;)? CPU.+Mac OS X/)) {
            $scope.isIos = true;
            $scope.localyyzt = "打开";
        ...
        // 查询膳食是否收藏
        $scope.collectFlag = false;
        XywyService.save("/member/collect/determineCollectFlag.json", { userId: openid, collectDataPid: $stateParams.pid, collectType: "SSYS" })
            .then(function (data) {
        ...
        $scope.addShouCang = function () {
            XywyService.save("/member/collect/addCollect.json", { userId: openid, collectDataPid: $stateParams.pid, collectType: "SSYS" })
                .then(function (data) {
        ...
        $scope.cancelShouCang = function () {
            XywyService.save("/member/collect/cancelCollect.json", { userId: openid, collectDataPid: $stateParams.pid, collectType: "SSYS" })
                .then(function (data) {
                    $scope.collectFlag = false;
                    Popup.alert("取消成功！")
                }, function (reason) {
    ...
    .controller('xwDetailC', function ($scope, $stateParams, XywyService, Popup, $window, GoZzJbYp, $state) {
        $scope.isIos = false;
        if (!!$window.navigator.userAgent.match(/\(i[^;]+;( U;)? CPU.+Mac OS X/)) {
            $scope.isIos = true;
            $scope.localyyzt = "打开";
        ...
        // 查询膳食是否收藏
        $scope.collectFlag = false;
        XywyService.save("/member/collect/determineCollectFlag.json", { userId: openid, collectDataPid: $stateParams.pid, collectType: "XWYS" })
            .then(function (data) {
                $scope.collectFlag = data.data.collectFlag;
        ...
        //添加收藏
        $scope.addShouCang = function () {
            XywyService.save("/member/collect/addCollect.json", { userId: openid, collectDataPid: $stateParams.pid, collectType: "XWYS" })
                .then(function (data) {
                    $scope.collectFlag = true;
                    Popup.alert("收藏成功！")
                }, function (reason) {
        ...
        //取消收藏
        $scope.cancelShouCang = function () {
            XywyService.save("/member/collect/cancelCollect.json", { userId: openid, collectDataPid: $stateParams.pid, collectType: "XWYS" })
                .then(function (data) {
                    $scope.collectFlag = false;
                    Popup.alert("取消成功！")
                }, function (reason) {
