angular.module('starter.ypznControllers', ['ionic'])
    ...
    .controller('ypznMainC', function ($ionicHistory, $ionicViewSwitcher, $scope, $state, $window) {
        //判断手机类型
        $scope.isIos = false;
        ...
        $ionicHistory.clearCache(["jtyy"]); //清除药品库
        // 跳转家庭用药
     ...
     // 跳转安全用药
        $scope.goWyp = function () {
        	$state.transitionTo('yuyinjiaohuclick', { gn: 'WYP', zhuci: '' }, { reload: true, notify: true });
        };
    ...
    .controller('jtyyC', function (wxApi, $ionicViewSwitcher, $scope, $http, $state, $stateParams, $timeout, XywyService, Popup, Outlet, $ionicScrollDelegate, audioControl, $window, $rootScope, geoLocation, $ionicModal, $ionicLoading, $ionicPopup) {
        ...
        $scope.loadEjfl = function (yjfl, ejfl) {
            if (ejfl&&$scope.currentChoose[$scope.currtab].yjfl == yjfl) {
                $scope.currentChoose[$scope.currtab].yjfl = "";
    ...
    // 药品库
    .controller('ypkC', function ($ionicHistory, $ionicViewSwitcher, $scope, $state, $window) {
        //判断手机类型
        $scope.isIos = false;
        if (!!$window.navigator.userAgent.match(/\(i[^;]+;( U;)? CPU.+Mac OS X/)) {
            $scope.isIos = true;
        ...
        $ionicHistory.clearCache(["ypznMainSearch"]); //清除搜索页缓存
        $scope.goSearch = function () {
    ...
    .controller('ypznMainSearchC', function ($ionicViewSwitcher, $scope, $http, $state, $stateParams, XywyService,  $window, $ionicPopup, Yuyin,JsUtil) {
